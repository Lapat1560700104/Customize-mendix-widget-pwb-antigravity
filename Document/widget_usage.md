...
เอกสารฉบับนี้อธิบายสเต็ปขั้นตอนโดยละเอียดในการนำ **PwbDatePicker** ที่ได้รับการคอมไพล์แล้ว ไปติดตั้ง กำหนดค่า และแสดงผลในโปรแกรม **Mendix Studio Pro**
...
3. หลังจากคอมไพล์สำเร็จ ระบบจะสร้างโฟลเดอร์ `dist/` ขึ้นมาและบันทึกไฟล์แพ็กเกจไว้ที่:
    `pwbDatePicker/dist/1.0.0/pwb.PwbDatePicker.mpk`
...
3. คัดลอกไปวางไว้ภายในโฟลเดอร์ย่อยชื่อ **`widgets`** (ถ้าไม่มีโฟลเดอร์ `widgets` ให้ทำการสร้างโฟลเดอร์ใหม่ขึ้นมาได้เลย)
   ...
   [Mendix Project Root]/
   ├── widgets/
   │   └── pwb.PwbDatePicker.mpk  <-- วางไฟล์ที่นี่
   └── [ProjectName].mpr
   ```
...
3. เลื่อนหาหัวข้อชื่อกลุ่มการใช้งาน (หรือค้นหาจากช่องค้นหาพิมพ์ว่า `"PWB Advanced DatePicker"`)
...
1. แก้ไขไฟล์ `pwbDatePicker/package.json` ตรงการตั้งค่า `config.projectPath` ให้ชี้ไปยังที่ตั้งโฟลเดอร์ Mendix App ของคุณ:
