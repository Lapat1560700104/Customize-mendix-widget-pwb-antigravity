...
* **collapsible Option Grouping**: Collapses dropdown search options into structured, expandable categories/folders based on a grouping attribute.
* **DOM Virtualization**: High-performance option lists rendering only visible dropdown rows to protect browser DOM from clogging when loading massive databases.
* **Native Mendix Event Triggers**: Triggers standard Mendix actions (microflows, nanoflows) on selection change (`onChange`), focus (`onEnter`), blur (`onLeave`), and as the user types queries (`onFilterChange`).
* **Interactive Design & Aesthetics**: Customizable glassmorphism depth, accent theme colors, and popover scroll dimensions.
...
* **ระบบจัดกลุ่มหมวดหมู่ (Collapsible Grouping)**: จัดกลุ่มผลลัพธ์ในกล่องดรอปดาวน์แยกออกเป็นหัวข้อหลักที่ยุบและขยายหมวดหมู่เพื่อความสวยงามและเป็นระเบียบได้
* **การโหลดหน้าเว็บประสิทธิภาพสูง (DOM Virtualization)**: แสดงผลเฉพาะแถวที่ผู้ใช้มองเห็นในช่องหน้าต่างดร็อปดาวน์จริง ช่วยให้สามารถโหลดตัวเลือกขนาดใหญ่หลักพันหลักหมื่นรายการได้โดยไม่เกิดอาการหน่วงหรือบราวเซอร์ค้าง
* **การเรียกใช้ Action ตอบสนองการกระทำ (Event Triggers)**: สั่งคอมมิทค่าหรือรัน Microflow/Nanoflow ทันทีเมื่อเปลี่ยนค่าตัวเลือก (`onChange`), เมื่อนำเมาส์โฟกัสเข้า (`onEnter`), เมื่อโฟกัสออกจากกล่อง (`onLeave`) และเมื่อผู้ใช้กำลังพิมพ์ข้อความฟิลเตอร์ (`onFilterChange`)
