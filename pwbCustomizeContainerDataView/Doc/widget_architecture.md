...
# pwbCustomizeContainerDataView (v1.0.0)
...
### Phase 3 — Drag Event Lifecycle (วงจรการลาก-วาง)

```
ผู้ใช้เริ่มลาก Row Item (mousedown + move > 5px)
         │
         ▼
onDragStart(e, index):
  • setDraggingIndex(index)    → เปิด class "pwb-dragging" (opacity ลด, scale ย่อ)
  • setDragImage → ใช้ 1x1 transparent GIF → ซ่อน browser ghost image เริ่มต้น
  • e.dataTransfer.effectAllowed = "move"
         │
         ▼
ผู้ใช้ลากผ่าน Row อื่น:
onDragOver(e, targetIndex):
  • e.preventDefault()         → อนุญาตให้ Drop ได้ (ถ้าไม่ preventDefault จะ block)
  • setDragOverIndex(targetIndex) → เปิด class "pwb-drag-over" (glow border indicator)
         │
         ▼
ผู้ใช้ปล่อย (mouseup):
onDrop(targetIndex):
  • splice(draggingIndex, 1) → ดึง item ออกจากตำแหน่งเดิม
  • splice(targetIndex, 0, movedItem) → ใส่ item ลงในตำแหน่งใหม่
  • setOrderedItems(nextItems) → อัปเดต UI ทันที (Optimistic Update)
  • onOrderChange(nextItems.map(i => i.id)) → ส่ง GUID array ใหม่ขึ้นไป
         │
         ▼
onDragEnd():
  • reset draggingIndex = null
  • reset dragOverIndex = null
  → ลบ class "pwb-dragging" และ "pwb-drag-over" ทั้งหมด
```
...
## ⚡ Performance Characteristics (ลักษณะด้านประสิทธิภาพ)
...
| **Optimistic Update** | UI อัปเดตทันทีก่อน Mendix บันทึก | ไม่มี visual lag แม้ Microflow ใช้เวลา |
| **useMemo Hydration** | คำนวณ dragItems ใหม่เฉพาะเมื่อ items หรือ sortedValue เปลี่ยน | ไม่ re-compute ทุก render |
| **No external Library** | ใช้ HTML5 Drag API ล้วนๆ | Bundle size เล็ก, ไม่มี dependency conflict |
| **Transparent drag image** | ใช้ 1x1 GIF แทน browser ghost | ไม่กระตุกระหว่างลาก |
